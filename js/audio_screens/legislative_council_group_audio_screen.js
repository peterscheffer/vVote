 ...
 *
 *
 * LegislativeCouncilGroupAudioScreen - The audio screen class for the group council ballot for the blind interface.
 * 
 * @author Peter Scheffer
 */
 
var LegislativeCouncilGroupAudioScreen = AudioScreen.extend({  
  init: function () {
    this._super();
  },
  ...
  initialEntry: function () {    
    ...
    var callback1 = function () {
      ...
      var instructions = new AudioInstructions(['legislative_council_atl_gestures'], true, true, callback2);     
      var container = getContainer();
      var audioController = container.Resolve("audioController");
    ...
    var clips = [ ['legislative_council_ballot_for'],
                  ...
                  ['legislative_council_atl_gestures'], 
                  ['atl_to_btl_switch'],
    ...
    if (group.getIsUngrouped()) {
      ...
      clips = clips.concat(['gvs_ungrouped_candidates_information']);
    ...
    // If this group doesn't have a ballot box (effectively it has no group voting ticket).
      ...
      clips = clips.concat(['gvs_unnamed_group_information']);
    ...
    clips = clips.concat([ ['change_vote_later'], ['press_three_fingers_for_options_menu'] ]);
  ...
  entryFromInformalWarningScreen: function () {
    ...
    clips = clips.concat(['press_three_fingers_for_options_menu']);
  ...
  selectCurrentGroup: function (group, error) {
    ...
    if (group.getIsUngrouped()) {
      ...
      clips = [ ['gvs_ungrouped_candidates_information'] ];

    // If this group doesn't have a ballot box (effectively it has no group voting ticket).
    } else if (!group.getHasBallotBox() && group.getName() != null && group.getName() != "") {

      clips = [ ['group_has_no_ticket'] ];

    // If this group doesn't have a ballot box (effectively it has no group voting ticket).
    } else if (!group.getHasBallotBox()) {

      clips = [ ['gvs_unnamed_group_information'] ];
